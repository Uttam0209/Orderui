...
import {
  GetEquipmentRentalDetailWithOrderNoTypeSP,
...
const AdditionalEquipView = () => {
  ...
  useEffect(() => {
    if (userlog === undefined || userlog === null) {
      ...
      if (localStorage.getItem("user_name") === "Admin") {
        GetAudioDetailWithOrderNoAllSP("ALL", "EQUIPMENT").then((res) => {
  ...
  const ViewDownloadModal = (invoice) => {
    setDownloadModal([]);
    setOrderid(invoice);
    GetEquipmentRentalDetailWithOrderNoTypeSP(invoice).then((res) => {
  ...
  const ClickPaynow = (e, invoiceid, mTotal) => {
    ...
    var myuserData = `${BRN}|${mTotal}|https://orderpi.pavilionsinteriors.com/#/paymantreciept|${localStorage.getItem(
      "contact_name"
    )}|${localStorage.getItem(
      "user_id"
    )}|${invoiceid}|EQUIPMENT|${localStorage.getItem(
      "user_country"
    )}|${localStorage.getItem("user_email")}|${token}|999999`;

    GetEncryptData(myuserData).then((res) => {
      window.open(
        `https://payments.pavilionsinteriors.com/TrnReq.aspx?id=${res.enString}`,
        //`https://localhost:44395/TrnReq.aspx?id=${res.enString}`,
    ...
    <>
      ...
      <div className="content-wrapper">
        <div className="container px-lg-4 px-3">
          <div className="justify-content-between  align-items-baseline d-flex py-3 flex-lg-row flex-column">
            <h2>Additional Equipment</h2>
            {localStorage.getItem("user_name") === "Admin" ? (
              <></>
            ) : (
              <Link
                to="/additionalequipment"
                                    ...
                                    return (
                                      <tr key={i}>
                                        ...
                                        <td className="text-end">
                                          ...
                                          {Math.round(x.gTotalAmount).toFixed(
                                            0
                                          )}
                                        </td>
                                      </tr>
...
export default AdditionalEquipView;
