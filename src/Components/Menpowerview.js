...
import {
  ...
  GetTemporaryStaffDetailWithOrderNoTypeSP,
...
const Menpowerview = () => {
  ...
  useEffect(() => {
    if (userlog === undefined || userlog === null) {
      ...
      if (localStorage.getItem("user_name") === "Admin") {
        GetAudioDetailWithOrderNoAllSP("ALL", "MANPOWER").then((res) => {
          setData(res.data);
        });
      } else {
        GetAudioDetailWithOrderNoAllSP(
  ...
  const ViewDownloadModal = (invoice) => {
    setDownloadModal([]);
    setOrderid(invoice);
    GetTemporaryStaffDetailWithOrderNoTypeSP(invoice).then((res) => {
        ...
        <div className="container px-lg-4 px-3">
          <div className="justify-content-between  align-items-baseline d-flex py-3">
            ...
            {localStorage.getItem("user_name") === "Admin" ? (
              ...
              <Link to="/Menpower" className="text-white btn btn-info">
                Add Men Power
              </Link>
            )}
          </div>
