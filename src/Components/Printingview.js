...
import {
  ...
  GetPrintingDetailWithOrderNoTypeSP,
...
const Printingview = () => {
  ...
  useEffect(() => {
    if (userlog === undefined || userlog === null) {
      ...
      if (localStorage.getItem("user_name") === "Admin") {
        GetAudioDetailWithOrderNoAllSP("ALL", "PRINTING").then((res) => {
          setData(res.data);
        });
      } else {
        GetAudioDetailWithOrderNoAllSP(
  ...
  const ViewDownloadModal = (invoice) => {
    setDownloadModal([]);
    setOrderid(invoice);
    GetPrintingDetailWithOrderNoTypeSP(invoice).then((res) => {
  ...
  const ClickPaynow = (e, invoiceid, mTotal) => {
    ...
    const BRN = `${localStorage.getItem(
      "user_id"
    )}/${curr.getDate()}${curr.getHours()}${curr.getMinutes()}${curr.getSeconds()}`;
    var myuserData = `PG_ClientID=defexpo2022|PG_ClientRefID=${BRN}|PG_Amount=${mTotal}|PG_ReturnURL=http://orders.pavilionsinteriors.com/paymantreciept|
    PG_PayeeFirstname=${localStorage.getItem(
      "contact_name"
    )}|PG_PayeeLastname=${localStorage.getItem(
      "user_id"
    )}|PG_PayeeAddress=${invoiceid}|PG_PayeePincode=123456|PG_PayeeCity=PRINTING|PG_PayeeState=|
    PG_PayeeCountry=${localStorage.getItem(
      "user_country"
    )}|PG_Email=${localStorage.getItem("user_email")}|PG_Contact=0000000000`;

    GetEncryptData(myuserData).then((res) => {
      window.open(
        `https://www.tecogis.com/defpg/Payment/TecogisPaymentApi?PaymentData=${res.enString}`,
        "_blank"
      );
    });
  };
        ...
        <div className="container px-lg-4 px-3">
          <div className="justify-content-between  align-items-baseline d-flex py-3">
            ...
            {localStorage.getItem("user_name") === "Admin" ? (
              ...
              <Link to="/printing" className="text-white btn btn-info">
                Add Printing
              </Link>
            )}
          </div>
                    ...
                    <tr>
                      ...
                      {localStorage.getItem("user_name") === "Admin" ? (
                        <>
                          ...
                          <th className="text-end">stallNo</th> <th>GST No.</th>
                          <th>PAN No.</th>
                        </>
                      ) : (
                        <></>
