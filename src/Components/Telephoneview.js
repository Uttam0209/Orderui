...
import {
  ...
  GetTelephoneDetailWithOrderNoTypeSP,
...
const Telephoneview = () => {
    ...
    if (userlog === undefined || userlog === null) {
      ...
      if (localStorage.getItem("user_name") === "Admin") {
        GetTelephoneData().then((res) => {
          setAllData(res.data);
          GetAudioDetailWithOrderNoAllSP("ALL", "INTERNET").then((res) => {
            setData(res.data);
          });
        });
      } else {
  ...
  const ViewDownloadModal = (invoice) => {
    setDownloadModal([]);
    setOrderid(invoice);
    GetTelephoneDetailWithOrderNoTypeSP(invoice).then((res) => {
  ...
  const ClickPaynow = (e, invoiceid, mTotal) => {
    ...
    var myuserData = `${BRN}|${mTotal}|https://orders.pavilionsinteriors.com/#/paymantreciept|${localStorage.getItem(
      ...
      "user_id"
    )}|${invoiceid}|INTERNET|${localStorage.getItem(
      "user_country"
        ...
        <div className="container px-lg-4 px-3">
          <div className="justify-content-between  align-items-baseline d-flex py-3">
            ...
            {localStorage.getItem("user_name") === "Admin" ? (
              ...
              <Link to="/telephoneinternet" className="text-white btn btn-info">
                Add Telephone/Internet
              </Link>
            )}
          </div>
          {data.length === 0 ? (
